...
| **unit-test-gen-agent** | 单元测试 | opus | 是 | 连接本地 DB 查真实数据，按 router 生成 pytest；已覆盖则跳过写入 | `tests/{router}/{router}_unit_test.py` + `_unit_data.yaml` |
...
线上 K8s、Loki、Prometheus 观测能力由 `kube-observability` MCP 提供，统一通过白名单只读工具查询。`feishu-doc-reader` 为文档读取与本地下载，不写入飞书侧数据。
...
| `tests/{router}/` | unit-test-gen-agent | Test Runner | 单元测试（按 `app/routers/<name>.py` 分目录） |
| `tests/{branch}/` | integration-test-gen-agent | Test Runner | 集成测试（xmind，按 branch 目录） |
...
| 类型 | 别名 | 生成方式 | Agent | 产出 |
|------|------|----------|-------|------|
| 自发性测试 | 单元测试 | 根据接口变动自动查询本地 DB 真实数据生成 | `unit-test-gen-agent` | `tests/{router}/{router}_unit_test.py`（已覆盖时可跳过写入） |
...
**典型流程**：`/fix` 判断为线上异常（路径 B），切换 plan 模式 → `kube-observability` MCP 拉取日志：`KeyError: 'transfer_status'` → `dev-mysql-bastion-query` Skill 比对 DB 确认根因（旧数据 transfer_status 为 NULL）→ 根因分析报告输出 → **等待用户确认** → Debugger 最小化修复（增加 None 防御）→ 本地验证 → commit
...
Monitor Agent 调用 `kube-observability` MCP，返回结构化报告。所有线上观测查询均通过 MCP 服务端白名单工具执行，不会误操作生产数据。
