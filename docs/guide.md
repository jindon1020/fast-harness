...
Monitor Agent 调用 `kubectl-readonly` + `prometheus-metrics-query` Skill，返回结构化报告。所有操作均为 RBAC 只读，不会误操作生产数据。
...
| kubectl-readonly | `plugin/skills/kubectl-readonly/` | K8s 只读 |
| k8s-monitor-full | `plugin/skills/k8s-monitor-full/` | K8s 诊断 |
