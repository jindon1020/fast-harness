...
| `unit_test` | `on`（默认）/ `off` / `<module>` | 控制单元测试开关，传模块名可限定测试范围 |
...
| `unit_test` | `on` | `off` 跳过 Phase 3；`<module>` 限定测试范围 |
...
| `unit_test` | `on` | `off` 跳过 Phase 3；`<module>` 限定范围 |
...
| `unit_test` | `on` | `off` 跳过回归单元测试；`<module>` 限定范围 |
...
| `unit_test` | `on` | `off` 跳过 Phase 1/3 单元测试基线与验证 |
