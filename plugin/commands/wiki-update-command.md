...
### 禁止行为（无论情况如何，一律适用）
- 禁止在 wiki 未初始化（MANIFEST.json 不存在）时跳过报错直接操作 wiki 文件
- 禁止自行判断问题类型而不向用户确认（无法确定类型时必须 AskQuestion）
- 禁止在未调用 code-wiki-gen Skill 的情况下直接写入或替换 wiki 内容
- 禁止省略 Pre-flight 的 MANIFEST.json 检测与受影响 sections 分析步骤
