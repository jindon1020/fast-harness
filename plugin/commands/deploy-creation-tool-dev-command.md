...
1. 查看 `git status --short` 和冲突文件，逐个分析冲突内容。
2. **优先自行处理**：当冲突结果在技术上明确（例如 import 顺序、格式化差异、一方为纯新增、明显的取最新等），直接解决，执行 `git add <已解决文件>`，必要时用 `git commit` 完成合并。
3. **不明确时通过 AskUserQuestion 向人类确认，禁止自行猜测**：如果任何冲突涉及业务/产品判断，或两边都是有意义的逻辑改动而无法确定取舍，使用 `AskUserQuestion` 工具向用户提问。每个提问需包含：
   - `header`: 冲突文件名（截断到 12 字符内）
   - `question`: 简述冲突点
   - `options`: 至少两个明确选项，例如「采用当前分支（`$SOURCE_BRANCH`）的版本」「采用 dev 的版本」「两者合并（说明如何合并）」
   收到用户选择后，按用户的决定解决该冲突，再 `git add` 并完成合并。
4. 如果用户在提问中选择中止，或冲突过于复杂无法通过选项表达，停止并报告未解决的文件，不要强行 `git commit`。

> ⚠️ 解决冲突只允许编辑发生冲突的具体文件并 `git add`，**禁止**使用 `git reset --hard`、`git checkout -- <file>`、`git clean` 等破坏性命令丢弃他人改动。
