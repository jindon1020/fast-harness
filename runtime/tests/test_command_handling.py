...
class CommandHandlingTest(unittest.TestCase):
    ...
    def test_bug_fix_form_supports_multi_repo_contexts(self):
        ui_dir = Path(__file__).resolve().parents[1].joinpath("ui")
        page = ui_dir.joinpath("bug-fix", "index.html").read_text()
        styles = ui_dir.joinpath("bug-fix", "styles.css").read_text()
        app = ui_dir.joinpath("bug-fix", "app.js").read_text()

        self.assertIn("关联仓库排查上下文", page)
        self.assertIn('id="repoContextList"', page)
        self.assertIn('id="btnAddRepoContext"', page)
        self.assertIn('id="primaryCorrelationName"', page)
        self.assertIn('<select id="occurredAt">', page)
        self.assertIn("近 1 小时", page)
        self.assertIn("近 3 小时", page)
        self.assertIn("近 3 天", page)
        self.assertIn("当前登录用户ID", page)
        self.assertNotIn("影响用户或数据 ID", page)
        self.assertIn("repo_contexts: repoContextsPayload()", app)
        self.assertIn("function addRepoContext", app)
        self.assertIn("function renderRepoContexts", app)
        self.assertIn("repo-context-row", styles)
        self.assertIn("repo-context-row__grid", styles)

    def test_bug_fix_form_supports_screenshot_attachments(self):
        ui_dir = Path(__file__).resolve().parents[1].joinpath("ui")
        page = ui_dir.joinpath("bug-fix", "index.html").read_text()
        styles = ui_dir.joinpath("bug-fix", "styles.css").read_text()
        app = ui_dir.joinpath("bug-fix", "app.js").read_text()

        self.assertIn('id="screenshotDropzone"', page)
        self.assertIn('id="screenshotInput"', page)
        self.assertIn("上传截图", page)
        self.assertIn("Ctrl+V 粘贴", page)
        self.assertIn("screenshot_images", app)
        self.assertIn("function handleScreenshotPaste", app)
        self.assertIn("function addScreenshotFiles", app)
        self.assertIn("function renderScreenshotPreview", app)
        self.assertIn("renderPersistedScreenshots", app)
        self.assertIn("screenshot-uploader", styles)
        self.assertIn("screenshot-preview", styles)
        self.assertIn("stage-screenshot-gallery", styles)

    def test_bug_fix_page_supports_developer_code_approval_commit_and_push(self):
        ui_dir = Path(__file__).resolve().parents[1].joinpath("ui")
        page = ui_dir.joinpath("bug-fix", "index.html").read_text()
        styles = ui_dir.joinpath("bug-fix", "styles.css").read_text()
        app = ui_dir.joinpath("bug-fix", "app.js").read_text()

        self.assertIn("研发代码改动审批", app)
        self.assertIn('id="btnCodeApprove"', app)
        self.assertIn('id="btnBugCommit"', app)
        self.assertIn('id="btnBugPush"', app)
        self.assertIn('id="commitModal"', page)
        self.assertIn("function openCommitModal", app)
        self.assertIn("function pushBugPipeline", app)
        self.assertIn('kind === "code" ? "code-approval" : "approval"', app)
        self.assertIn("/git/commit-message", app)
        self.assertIn("/git/commit", app)
        self.assertIn("/git/push", app)
        self.assertIn("code-review-panel", styles)

    def test_bug_fix_sidebar_pipeline_items_use_more_menu(self):
        ui_dir = Path(__file__).resolve().parents[1].joinpath("ui")
        page = ui_dir.joinpath("bug-fix", "index.html").read_text()
        styles = ui_dir.joinpath("bug-fix", "styles.css").read_text()
        app = ui_dir.joinpath("bug-fix", "app.js").read_text()

        self.assertIn('id="btnSidebarNewPipeline"', page)
        self.assertIn("新增 Bug 会话", page)
        self.assertIn("历史会话", page)
        self.assertIn("测试环境 Bug 排查流水线", page)
        self.assertIn("返回聊天工作台", page)
        self.assertIn('id="chatNavLink"', page)
        self.assertIn('data.user.role === "reporter"', app)
        self.assertIn("主题", page)
        self.assertIn("退出", page)
        self.assertIn('$("#btnSidebarNewPipeline").addEventListener("click", showCreateView)', app)
        self.assertIn("sidebar-new-button", styles)
        self.assertIn("pipeline-menu-button", app)
        self.assertIn("pipeline-menu", styles)
        self.assertIn("重命名会话", app)
        self.assertIn("删除会话", app)
        self.assertIn("function openPipelineRenameModal", app)
        self.assertIn("function openPipelineDeleteModal", app)
        self.assertIn('id="pipelineSessionModal"', page)
        self.assertIn('api("PATCH", `/bug-pipelines/${encodeURIComponent(pipelineId)}`', app)
        self.assertIn('api("DELETE", `/bug-pipelines/${encodeURIComponent(pipelineId)}`', app)

    def test_bug_fix_page_supports_pipeline_termination_and_clear_status_nodes(self):
        ui_dir = Path(__file__).resolve().parents[1].joinpath("ui")
        styles = ui_dir.joinpath("bug-fix", "styles.css").read_text()
        app = ui_dir.joinpath("bug-fix", "app.js").read_text()

        self.assertIn('id="btnTerminatePipeline"', app)
        self.assertIn("function openTerminateModal", app)
        self.assertIn("/terminate", app)
        self.assertIn("statusLabel", app)
        self.assertIn("node-status-badge", app)
        self.assertIn("border-left-width: 5px", styles)
        self.assertIn('data-status="running"', styles)
        self.assertIn('data-status="failed"', styles)
        self.assertIn('data-status="skipped"', styles)

    def test_bug_fix_quality_gate_steps_can_be_skipped_individually(self):
        ui_dir = Path(__file__).resolve().parents[1].joinpath("ui")
        styles = ui_dir.joinpath("bug-fix", "styles.css").read_text()
        app = ui_dir.joinpath("bug-fix", "app.js").read_text()

        self.assertIn('new Set(["code_review", "unit_test", "regression"])', app)
        self.assertIn("function skipStepButton", app)
        self.assertIn("function skipPipelineStep", app)
        self.assertIn("/skip", app)
        self.assertIn("node-skip", app)
        self.assertIn(".node-skip", styles)
